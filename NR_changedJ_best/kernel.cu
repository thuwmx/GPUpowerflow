...
//__shared__ double G[39*39],B[39*39];
...
__global__ void changeVAng2(double *V,double *Ang,int *FunctoNode,double *deltat,double *fx1,double *fx2){
	...
	//	       V[FunctoNode[i]]=V[FunctoNode[i]]+(V[FunctoNode[i]]-(*deltat)*fx1[i])*0.5*(*i)*(fx2[i]+fx1[i]);
	//-(*deltat)*fx1[i]
...
//	cudaDeviceReset();
	...
	for(int ite=0;ite<iteration;ite++){
		...
		//if((fp=fopen("csrLU39.txt","rt+"))==NULL){
		//     printf("Cannot open file strike any key exit!");
		//     getch();
		//     exit(1);
		//   }
