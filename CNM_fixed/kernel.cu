...
int main()
{
	...
	*deltat=0.81;
	...
	FILE *fp;
	if((fp=fopen("net_13659.txt","rt+"))==NULL){
	...
	*anglelast=0;
	...
	while (t<tmax){
		...
		cusparseDcsrsv2_solve(handle, trans, nfunc, unz, &alpha, descrU, d_valU, d_rowptrU, d_colindU, infoU, d_z, dev_fx2, policy, pBuffer);
		cudaThreadSynchronize();
		// 	cudaMemcpy(test, dev_fx2,nfunc*sizeof(double), cudaMemcpyDeviceToHost);
		// 	for (int i=0;i<nfunc;i++)
		//printf("%f\n",test[i]);
		...
		cudaMemcpy(pf, dev_fx2,nfunc*sizeof(double), cudaMemcpyDeviceToHost);
		//QueryPerformanceCounter(&tend);
		//tsolve+=(tend.QuadPart - tstart.QuadPart)*1000.0/tc.QuadPart;
		...
		cudaMemcpy(dev_fx2, permuPf,nfunc*sizeof(double), cudaMemcpyHostToDevice);
		//cudaMemcpy(test, dev_fx2,nfunc*sizeof(double), cudaMemcpyDeviceToHost);
		//for (int i=0;i<nfunc;i++)
		//	printf("fx[%d]=%f\n",i,test[i]);
