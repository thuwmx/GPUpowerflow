...
//	cudaDeviceReset();
	...
	const int N=9241;
	int n=N-1;
	double tLU=0,tanaly=0,tpf=0,tsolve=0,tformY=0,tformJ=0,tchange=0;
	double t;
	int iteration=1;
	for(int ite=0;ite<iteration;ite++){
		...
		*anglelast=0;
		...
		while (t<tmax){
			...
			cudaMalloc((void**)&d_colindL,  lnz*sizeof(int));
			err=cudaMalloc((void**)&d_valU,  unz*sizeof(double));
		    err=cudaMalloc((void**)&d_colindU,  unz*sizeof(int));
			err=cudaMemcpy(d_valL, lx, lnz*sizeof(double), cudaMemcpyHostToDevice);
